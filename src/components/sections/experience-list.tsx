      ...
      <Container>
        <div className="grid gap-5">
          {experienceItems.map((item) => (
            <Card className="grid gap-6 md:grid-cols-[0.75fr_1.25fr]" key={`${item.company}-${item.dates}`}>
              <div>
                <p className="text-sm font-black uppercase tracking-[0.18em] text-ember">{item.dates}</p>
                <h2 className="mt-3 font-display text-3xl font-black tracking-[-0.04em] text-ink">{item.company}</h2>
